...
import { extractTitle, exportContentAsText } from "@shared/utils/note";
import { Card, CardHeader, CardTitle, CardContent } from "../components/ui/card";
import { Input } from "../components/ui/input";
import { Label } from "../components/ui/label";
import { Button } from "../components/ui/button";
import { SearchResults } from "../components/SearchResults";
import { searchResponseSchema, type SearchResponse } from "@shared/types/search";
...
export default function HomePage() {
  ...
  const getSearchResults = async () => {
    if (searchQuery) {
      setIsAiLoading(true);
      try {
        const response = await fetch(`/api/search?prompt=${encodeURIComponent(searchQuery)}`);
  ...
  useEffect(() => {
    // Set default note if not exists
  ...
  return (
    <div className="mb-12 p-4 flex min-h-[100svh] flex-col items-center sm:px-5 pt-[calc(10vh)] md:mb-0">
      <div className="flex flex-col">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="mt-4 text-gray-600 max-w-md">
            ...
            A simple, minimal AI powered note taking app and markdown editor - Built local-first, with cloud sync and real-time collaboration.
          </div>
        </div>
      ...
      {searchResults && <SearchResults searchResponse={searchResults} />}
