...
const extractTitle = (value: Value) => {
  ...
  for (const contentItem of contentArray) {
    if (contentItem.type === 'heading' && contentItem.content) {
      for (const innerContent of contentItem.content) {
        if (innerContent.type === 'text' && innerContent.text) {
          return innerContent.text.length > 40
            ? innerContent.text.substring(0, 40) + '...'
            : innerContent.text;
        }
      }
    }
  ...
  return (
    <ResponsiveDrawer>
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="bg-white flex flex-col rounded-t-[10px] h-[96%] md:h-full md:w-[400px] w-full mt-24 fixed bottom-0 right-0">
          <>
            <div className="rounded-t-[10px] flex flex-col md:flex-row h-full">
