    ...
    return (
        <div className="flex h-screen overflow-hidden transition-colors duration-300" style={style}>
            ...
            >
                ...
                {children}
                {/* SMFS toggle — top-right of main area, hidden on mobile (panel is md-only) */}
                {isAuthenticated && !smfs.isOpen && (
                    <button
                        onClick={smfs.togglePanel}
                        className="hidden md:block fixed top-4 right-4 z-30 p-2 rounded-lg bg-[var(--color-card)] border border-[var(--color-border-warm)] shadow-sm text-[var(--color-ink-muted)] hover:bg-[var(--color-sidebar-active)] transition-colors"
                        aria-label="Open Supermemory FS"
                        title={`Supermemory FS (${isMac ? "⌘⇧F" : "Ctrl+Shift+F"})`}
                    >
                        <PanelIcon size={18} />
                    </button>
                )}
